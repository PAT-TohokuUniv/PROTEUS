...
contains
  ...
  function ch_identify(spl, ich, type, in_reactant, in_product)
    ...
    if (spl%reaction_type_char(ich) == type) then
      ...
      allocate(reactants(spl%reactant_list(ich,1)),  products(spl%product_list(ich,1)))
      ...
      if (spl%product_list(ich,1) == 1 .and. size(in_product) == 1) then 
      ...
      else if (spl%product_list(ich,1) == 2 .and. size(in_product) == 2) then 
        if (    trim(ADJUSTL(reactants(1)))==trim(ADJUSTL(in_reactant(1))) &
      ...
      else if (spl%product_list(ich,1) == 3 .and. size(in_product) == 3) then 
        if (    trim(ADJUSTL(reactants(1)))==trim(ADJUSTL(in_reactant(1))) &
        & .and. trim(ADJUSTL(products(1)))==trim(ADJUSTL(in_product(1))) &
