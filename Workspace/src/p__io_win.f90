...
contains
  ...
  subroutine p__io_stable__fin(set, spl, var, grd)
    ...
    outdir = trim(ADJUSTL(set%dir_name_win))//'\output'
    write(command,*) 'mkdir ', trim(outdir)
    call system(command)

    outdir = trim(ADJUSTL(set%dir_name_win))//'\output\density'
    write(command,*) 'mkdir ', trim(outdir)
    call system(command)
    ...
    if (set%mode == '1D') then 
      ...
      outdir = trim(ADJUSTL(set%dir_name_win))//'\output\density\vmr'
      write(command,*) 'mkdir ', trim(outdir)
      call system(command)

      outdir = trim(ADJUSTL(set%dir_name_win))//'\output\flux'
      write(command,*) 'mkdir ', trim(outdir)
      call system(command)

      outdir = trim(ADJUSTL(set%dir_name_win))//'\output\rate'
      write(command,*) 'mkdir ', trim(outdir)
      call system(command)

      fname = trim(ADJUSTL(set%dir_name_win))//'\output\density\num\*.dat'
      write(command,*) 'del /Q ', trim(fname)
      call system(command)

      fname = trim(ADJUSTL(set%dir_name_win))//'\output\density\vmr\*.dat'
      write(command,*) 'del /Q ', trim(fname)
      call system(command)

      fname = trim(ADJUSTL(set%dir_name_win))//'\output\flux\*.dat'
      write(command,*) 'del /Q ', trim(fname)
      call system(command)

      fname = trim(ADJUSTL(set%dir_name_win))//'\output\rate\*.dat'
      write(command,*) 'del /Q ', trim(fname)
      call system(command)
      ...
      do isp = 1, spl%nsp
        ...
        fname = trim(ADJUSTL(set%dir_name))//'/output/density/vmr/vmr_'&
        &  //trim(ADJUSTL(spl%species(isp)))//'.dat'
        open(11, file = fname, status = 'replace' )
          do iz = 1, grd%nz
        ...
        if (spl%label_fix(isp) == 0) then
          ...
          fname = trim(ADJUSTL(set%dir_name))//'/output/flux/'&
          &  //trim(ADJUSTL(spl%species(isp)))//'.dat'
          open(11, file = fname, status = 'replace' )
            do iz = 1, grd%nz+1
              if (iz==1) then 
          ...
          fname = trim(ADJUSTL(set%dir_name))//'/output/flux/v_'&
          &  //trim(ADJUSTL(spl%species(isp)))//'.dat'
          open(11, file = fname, status = 'replace' )
            do iz = 1, grd%nz+1
              if (iz==1) then 
          ...
          fname = trim(ADJUSTL(set%dir_name))//'/output/flux/D_'&
          &  //trim(ADJUSTL(spl%species(isp)))//'.dat'
          open(11, file = fname, status = 'replace' )
            do iz = 1, grd%nz
      ...
      do isp = 1, spl%nsp_i
        ...
        fname = trim(ADJUSTL(set%dir_name))//'/output/rate/P_'&
        &  //trim(ADJUSTL(spl%species(jsp)))//'.dat'
        open(11, file = fname, status = 'replace' )
          do iz = 1, grd%nz
        ...
        fname = trim(ADJUSTL(set%dir_name))//'/output/rate/L_'&
        &  //trim(ADJUSTL(spl%species(jsp)))//'.dat'
        open(11, file = fname, status = 'replace' )
          do iz = 1, grd%nz
      ...
      outdir = trim(ADJUSTL(set%dir_name_win))//'\output\density\2DLAT'
      write(command,*) 'mkdir ', trim(outdir)
      call system(command)

      fname = trim(ADJUSTL(set%dir_name_win))//'\output\density\2DLAT\*.dat'
      write(command,*) 'del /Q ', trim(fname)
      call system(command)
  ...
  subroutine p__io_timeseries__fin(set, grd, spl, iout) ! in
    ...
    outdir = trim(ADJUSTL(set%dir_name_win))//'\output\timeseries'
    write(command,*) 'mkdir ', trim(outdir)
    call system(command)

    fname = trim(ADJUSTL(set%dir_name_win))//'\output\timeseries\*.dat'
    write(command,*) 'del /Q ', trim(fname)
    call system(command)
  ...
  subroutine p__io_rotation__fin(set, spl, var, grd)
    ...
    if (set%mode == '2DROT') then 
      do isp = 1, spl%nsp
        fname = trim(ADJUSTL(set%dir_name))//'/output/density/'&
        &  //trim(ADJUSTL(set%mode))//'/'//trim(ADJUSTL(spl%species(isp)))//'.dat'
        open(11, file = fname, status = 'replace' )
          do ix = 1, grd%nx
    ...
    if (set%mode == '3DROT') then 
      fname = trim(ADJUSTL(set%dir_name))//'/output/density/'&
      &  //trim(ADJUSTL(set%mode))//'/density.dat'
    ...
    fname = trim(ADJUSTL(set%dir_name))//'/output/density/'&
    &  //trim(ADJUSTL(set%mode))//'/grid.dat'
    open(11, file = fname, status = 'replace' )
      write(11, *) grd%nx, grd%ny, grd%nz
    close(11)
